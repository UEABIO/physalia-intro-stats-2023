...
> The normality assumption mainly affects standard errors, confidence intervals, and p-values.

> For moderate sample sizes, small deviations from normality are rarely problematic.

- Equal variances: In two-group comparisons, unequal variances are common and not automatically a concern. What matters is whether the difference in variability is large enough to distort uncertainty estimates.

> Unequal variances do not bias the estimated means or their difference.

> They can, however, affect standard errors and therefore inference.
...
#
# Interpretation:
#   - We look for large, systematic departures from the diagonal line
...
# A roughly constant spread suggests the assumption is reasonable;
# strong patterns or funnel shapes suggest heteroscedasticity.
...
Once we have satisfied ourselves that there are no severe violations of model assumptions, we proceed to interpret the fitted model.
