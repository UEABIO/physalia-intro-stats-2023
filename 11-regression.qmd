...
## Activity 5: Assumption checking
...
There is an issue with non-normality, heteroscedasticity and linearity - there is also one outlier.

```{r}

check_normality(janka_ls1)
check_heteroscedasticity(janka_ls1)
check_outliers(janka_ls1)

```

In this instance - while we could approach by bootstrapping, the most severe issue appears to be heteroscedasticity and curvature in the fit of the model. If we can deal with these issues directly we may be able to model the data more effectively. 
...
There are multiple ways to approach a non-linear relationship - one method is to use a series of transformations. The `boxCox()` function models likelihood under different transformations of the dependent variable. Here it indicates a square root transformation may be appropriate. 
...
janka_ls2 <- lm(sqrt(hardness) ~ dens, data = janka)

```

When we recheck the model, we see it is much improved, though heteroscedasticity remains an issue. This could lead to standard errors that are too narrow and incorrect p-values. 

```{r}

check_model(janka_ls2,
            detrend = FALSE)
...
check_normality(janka_ls2)

check_heteroscedasticity(janka_ls2)

```
### Robust errors

One way to adjust standard errors is to use the sandwich package, which actually handles the standard error correction behind the scenes for most of the other approaches we’ll look at. `sandwich` comes with a bunch of standard error correction functions, like `vcovHC()` for heteroscedasticity-consistent (HC) errors, `vcovHAC()` for heteroscedastiticy- and autocorrelation-consistent (HAC) errors, and `vcovCL()` for clustered errors (see their [website for all the different ones](http://sandwich.r-forge.r-project.org/articles/sandwich.html)).

#### When to Use:

- You suspect heteroscedasticity, but don’t know its structure

- You trust your model's form (e.g. linearity, correct predictors)

- You want robust inference without changing your model

#️### What it Does:

- Leaves your model unchanged

- Adjusts only the standard errors, using a robust (heteroscedasticity-consistent) covariance matrix

```{r}
library(sandwich)
library(lmtest)
robust_model <- coeftest(janka_ls2, type = "HC1")

```

If you want to use the emmeans package - it does not accept robust models as standard, but the same variance-covariance matrix can be applied directly:

```{r}
emmeans::emmeans(janka_ls2, 
                 specs = ~ dens, 
                 vcov. = vcovHC(janka_ls2,
                                type = "HC1"))
...
We could also consider weighted least squares (WLS) as an alternative. Unlike robust errors, WLS directly models the unequal variance by assigning weights to observations, allowing the regression to place more trust in data points with lower variability, which can improve the efficiency and accuracy of both estimates and standard errors.

`gls()` assumes that the heteroscedasticity follows a predictable pattern tied to one or more predictors.
If there’s no clear structure to the variance (i.e., residual spread is random), `gls()` may not help — and may even overfit or mislead. 

In this instance we suspect that heteroscedasticity increases as density changes.

#### When to Use:

- You know or can estimate how residual variance changes

- You want to improve efficiency (more precise coefficient estimates)

- You want to model the heteroscedasticity directly

#️### What it Does:

- Assigns weights to observations (lower for high-variance points)

- Adjusts both estimates and standard errors

- Requires that weights be calculated

```{r}

model <- nlme::gls(sqrt(hardness) ~ dens, data = janka, 
                   weights = nlme::varFixed(~ dens))
