...
More complex models can be compared to simpler ones and reported with F-tests as to whether they explain a significant amount of variance:

```{r}

full_model <- lm(longevity ~ type + thorax + sleep + type:sleep, data = dros_clean)

reduced_model <- lm(longevity ~ type + thorax + sleep, data = dros_clean)

anova(reduced_model, full_model)

```

Our initial model included an interaction term for type and sleep, but this was removed from the final model as it did not explain a statistically significant amount of variance F~2,118~ = 0.51, p = 0.6. 
